...
const CompanyStats = ({
  clientId, companyImageSrc, companyRating, companyName, location, title, summary, registrationDate, hires, jobsPosted,
}) => (
  <Box className='custom-box contact-center-info-root'>
    ...
    <div className='display-inline-flex justify-between is-fullwidth'>
      <div>
        ...
        <h4 className='h4 mt-20'>{ formatCount(hires) }</h4>
        <p className='para'> Hires </p>
      </div>
      <div>
        <h4 className='h4'> 2M+ </h4>
        <p className='para'> Total Calls </p>
        <h4 className='h4 mt-20'>{ formatCount(jobsPosted) }</h4>
