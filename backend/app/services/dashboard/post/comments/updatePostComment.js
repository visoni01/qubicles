...
export class UpdatePostCommentService extends ServiceBase {
  get constraints () {
    return constraints
  }
  ...
  async run () {
    try {
      ...
      await updateStatusPostComment({ user_activity_id, updatedCommentText: data.updatedCommentText })
      return {
        userActivityId: activityData.user_activity_id,
        message: MESSAGES.POST_COMMENT_DELETED_SUCCESSFULLY
      }
    } catch (error) {
      logger.error(`${getErrorMessageForService('UpdatePostCommentService')} ${error}`)
